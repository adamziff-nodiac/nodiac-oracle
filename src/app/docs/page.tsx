...
function CodeBlock({ children, title }: { children: string; title?: string }) {
  return (
    <div className="rounded-lg overflow-hidden border border-gray-200 dark:border-white/10">
      {title && (
        <div className="px-4 py-2 bg-gray-100 dark:bg-white/5 border-b border-gray-200 dark:border-white/10 text-xs text-gray-500 dark:text-gray-400 font-mono">
          {title}
        </div>
      )}
        ...
        <main className="flex-1 min-w-0 max-w-4xl space-y-16">
          ...
          <div>
            <div className="flex items-center gap-3 mb-4">
              ...
              <span className="text-gray-400 dark:text-gray-600">/</span>
              <span className="text-sm text-gray-500 dark:text-gray-400">Developer Docs</span>
            </div>
            <h1 className="text-4xl sm:text-5xl font-black text-gray-900 dark:text-white tracking-tight">
              Developer Documentation
            </h1>
            <p className="mt-4 text-lg text-gray-600 dark:text-nodiac-dusty-lilac max-w-2xl leading-relaxed">
          ...
          <Section id="architecture" title="Architecture Overview">
            <p>
              The system has three layers: a <strong className="text-gray-900 dark:text-white">data pipeline</strong> (Python, runs offline),
              a <strong className="text-gray-900 dark:text-white">Next.js 15 app</strong> (Bun runtime, Tailwind v4), and
              a <strong className="text-gray-900 dark:text-white">Supabase backend</strong> (Postgres + Auth).
            </p>
...
└── build-real-county-scores.py   # Offline data pipeline`}</CodeBlock>
            <p>
              <strong className="text-gray-900 dark:text-white">Key design decision:</strong> All weight changes happen client-side.
              The API returns raw 0–1 scores per criterion per county. The frontend computes composite scores
              in <code className="text-nodiac-secondary bg-gray-100 dark:bg-white/5 px-1 rounded">useMemo</code> — moving a
          ...
          <Section id="data-pipeline" title="Data Pipeline">
            <p>
              The Python script <code className="text-nodiac-secondary bg-gray-100 dark:bg-white/5 px-1 rounded">scripts/build-real-county-scores.py</code> downloads
            ...
            <SubSection title="Pipeline Steps">
              <ol className="list-decimal list-inside space-y-2 ml-2">
                <li><strong className="text-gray-900 dark:text-white">FIPS crosswalk</strong> — Downloads county name → FIPS code mapping (3,136 entries)</li>
                <li><strong className="text-gray-900 dark:text-white">EIA Form 861</strong> — Downloads ZIPs for 2013–2024 (12 years) → processes <code className="text-nodiac-secondary bg-gray-100 dark:bg-white/5 px-1 rounded">Service_Territory_2024.xlsx</code> for co-op density and <code className="text-nodiac-secondary bg-gray-100 dark:bg-white/5 px-1 rounded">Reliability_YYYY.xlsx</code> files for multi-year grid SAIDI averaging</li>
                <li><strong className="text-gray-900 dark:text-white">EIA Form 860</strong> — Downloads ZIP (21 MB, 13 files) → extracts solar/wind generators from Operable + Proposed sheets for curtailment proxy</li>
                <li><strong className="text-gray-900 dark:text-white">Census CBP</strong> — API calls for NAICS 5182, 5415, 517 + population estimates → labor score</li>
                <li><strong className="text-gray-900 dark:text-white">FCC BDC</strong> — ArcGIS Living Atlas query for county-level fiber availability (Dec 2024)</li>
                <li><strong className="text-gray-900 dark:text-white">Census ACS</strong> — API call for B28002 broadband subscriptions → fiber fallback</li>
                <li><strong className="text-gray-900 dark:text-white">Assembly</strong> — Joins all scores by FIPS, writes to <code className="text-nodiac-secondary bg-gray-100 dark:bg-white/5 px-1 rounded">public/data/county-scores.json</code> and Supabase <code className="text-nodiac-secondary bg-gray-100 dark:bg-white/5 px-1 rounded">county_scores</code> table</li>
              </ol>
            </SubSection>

            <SubSection title="Data Loading in the Frontend">
              <p>
                <code className="text-nodiac-secondary bg-gray-100 dark:bg-white/5 px-1 rounded">useCountyScores</code> tries
                the Supabase API first (<code className="text-nodiac-secondary bg-gray-100 dark:bg-white/5 px-1 rounded">/api/county-scores</code>),
                then falls back to the static JSON file if the API returns fewer than 2,000 rows (Supabase
                defaults to 1,000 row limit). The API route explicitly sets <code className="text-nodiac-secondary bg-gray-100 dark:bg-white/5 px-1 rounded">limit(5000)</code>.
              </p>
            </SubSection>
          </Section>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-200 dark:border-white/10">
                      ...
                      <th className="text-left py-2 pr-3 text-gray-500 dark:text-gray-400 font-medium">Calculation</th>
                      <th className="text-left py-2 text-gray-500 dark:text-gray-400 font-medium">Result</th>
                    </tr>
                  </thead>
                  <tbody className="font-mono text-xs">
                    <tr className="border-b border-gray-100 dark:border-white/5">
                      <td className="py-2 pr-3 text-gray-900 dark:text-white font-sans text-sm">Balanced</td>
                      <td className="py-2 pr-3 text-gray-600 dark:text-gray-300">(0.80+0.65+0.50+0.50+0.50+0.50)/6 × 10</td>
                      <td className="py-2 text-nodiac-secondary font-bold">5.75</td>
                    </tr>
                    <tr className="border-b border-gray-100 dark:border-white/5">
                      <td className="py-2 pr-3 text-gray-900 dark:text-white font-sans text-sm">Co-op Priority</td>
                      <td className="py-2 pr-3 text-gray-600 dark:text-gray-300">(0.80×3+0.65×1+0.50×1+0.50×2+0.50×0.5+0.50×1)/8.5 × 10</td>
                      <td className="py-2 text-nodiac-secondary font-bold">6.21</td>
                    </tr>
                    <tr>
                      <td className="py-2 pr-3 text-gray-900 dark:text-white font-sans text-sm">Curtailment Capture</td>
                      <td className="py-2 pr-3 text-gray-600 dark:text-gray-300">(0.80×1+0.65×1.5+0.50×3+0.50×1+0.50×0.5+0.50×1)/8 × 10</td>
            ...
            <SubSection title="Site Scoring & Tier Assignment">
              ...
              <div className="flex gap-4 mt-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm"><strong className="text-gray-900 dark:text-white">Strong Fit:</strong> ≥ 6.5</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="w-3 h-3 rounded-full bg-[#b48fc1]" />
                  <span className="text-sm"><strong className="text-gray-900 dark:text-white">Moderate Fit:</strong> ≥ 4.0</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="w-3 h-3 rounded-full bg-[#ef4444]" />
                  <span className="text-sm"><strong className="text-gray-900 dark:text-white">Weak Fit:</strong> &lt; 4.0</span>
                </div>
              </div>
...
// Anything below 4.0 is "Weak Fit" (bad)`}</CodeBlock>
          ...
          <Section id="six-criteria" title="The Six Criteria">
            <div className="space-y-8">
              ...
              <SubSection title="2. Grid Reliability (grid_reliability_score)">
                <p>Grid uptime measured by SAIDI (average outage minutes per customer per year). <strong className="text-gray-900 dark:text-white">Inverse metric</strong> — lower SAIDI = higher score. Uses <strong className="text-gray-900 dark:text-white">multi-year averaging</strong> for robustness.</p>
                <div className="bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-lg p-4 space-y-2 text-sm">
                  <p><strong className="text-gray-700 dark:text-gray-200">Source:</strong> EIA Form 861 Reliability Data (2013–2024, up to 12 years) — ~900–1,000 utilities report SAIDI per year. Prefers &ldquo;IEEE Without Major Event Days&rdquo; metric.</p>
                  <p><strong className="text-gray-700 dark:text-gray-200">Method:</strong> For each year, map utility SAIDI to service territory counties and average per county. Then average across all available years per county. Inverse percentile rank of multi-year average.</p>
                  <p><strong className="text-gray-700 dark:text-gray-200">Why multi-year:</strong> Single-year SAIDI can be skewed by one bad storm or one good year. Multi-year averages smooth out anomalies and better reflect structural grid quality.</p>
                  <p><strong className="text-gray-700 dark:text-gray-200">Coverage metadata:</strong> Each county includes <code className="text-nodiac-secondary bg-gray-100 dark:bg-white/5 px-1 rounded">grid_reliability_years</code> (number of years with data) and <code className="text-nodiac-secondary bg-gray-100 dark:bg-white/5 px-1 rounded">grid_reliability_data_range</code>. Counties with more years of data have more reliable scores.</p>
                  <p><strong className="text-gray-700 dark:text-gray-200">Coverage:</strong> ~3,025 of 3,143 counties (96%). Remaining 4% default to 0.5.</p>
                </div>
              </SubSection>
              ...
              <SubSection title="3. Clipped/Curtailed (clipped_curtailed_score)">
                ...
                <div className="bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-lg p-4 space-y-2 text-sm">
                  <p><strong className="text-gray-700 dark:text-gray-200">Source:</strong> EIA Form 860 (2024) — 8,684 variable renewable generators, 277,437 MW total.</p>
                  <p><strong className="text-gray-700 dark:text-gray-200">Method:</strong> Three-component composite:</p>
                  <div className="bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded px-3 py-2 font-mono text-xs text-nodiac-secondary">
                  ...
                  <p><strong className="text-gray-700 dark:text-gray-200">Coverage:</strong> 1,684 of 3,143 counties (54%). Remaining 46% score 0.0 (no renewables).</p>
                </div>
              </SubSection>
              ...
              <SubSection title="4. Permitting (permitting_score)">
                ...
                <div className="bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-lg p-4 space-y-2 text-sm">
                  ...
                  <p><strong className="text-gray-700 dark:text-gray-200">Sources:</strong> NCSL state incentive database, SDI Alliance policy tracker, H5 Data Centers 2025-2026 state rankings, NAIOP development reports, Data Center Watch moratorium tracker, state regulatory environment assessments.</p>
                  <p><strong className="text-gray-700 dark:text-gray-200">Methodology:</strong> Multi-component scoring: state incentive programs (30%), regulatory environment (25%), moratorium/opposition risk (25%), tax policy (20%). County-level adjustments applied where specific opposition or moratorium data exists (e.g., Loudoun County VA, multiple GA counties).</p>
                  <p><strong className="text-gray-700 dark:text-gray-200">Range:</strong> 0.30 (hostile, active moratoria) → 0.50 (neutral) → 0.85+ (welcoming with strong incentives). TX = 0.85, WY = 0.83, NJ = 0.39.</p>
                  <p><strong className="text-gray-700 dark:text-gray-200">Citations:</strong> Each county links to relevant source URLs via <code className="text-nodiac-secondary bg-gray-100 dark:bg-white/5 px-1 rounded">permitting_citation_ids</code> referencing the <code className="text-nodiac-secondary bg-gray-100 dark:bg-white/5 px-1 rounded">permitting_citation_registry</code> array in the data file.</p>
                </div>
              </SubSection>

              <SubSection title="5. Skilled IT Labor (labor_score)">
                <p>Existing tech business presence per capita — a <strong className="text-gray-900 dark:text-white">proxy for available talent</strong>, not a direct measure of hireable labor.</p>
                <div className="bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-lg p-4 space-y-2 text-sm">
                  <p><strong className="text-gray-700 dark:text-gray-200">Source:</strong> Census County Business Patterns (CBP) 2023 — counts <em>businesses</em> under NAICS 5182 (Data Processing &amp; Hosting), 5415 (Computer Systems Design), and 517 (Telecommunications). Population from Census 2024 vintage.</p>
                  <p><strong className="text-gray-700 dark:text-gray-200">Method:</strong> Sum employees across all three NAICS per county, divide by population for per-10K density, percentile rank normalize.</p>
                  <p><strong className="text-gray-700 dark:text-gray-200">What it measures:</strong> The density of existing tech/telecom businesses in a county. Counties with more data processing, IT consulting, and telecom firms are likely to have a deeper pool of relevant talent nearby.</p>
                  <p><strong className="text-gray-700 dark:text-gray-200">Limitations:</strong> This is a <em>proxy</em>. CBP counts business establishments, not individual workers available for hire. It doesn&apos;t capture remote workers, freelancers, or talent willing to relocate. A county with one large employer (e.g., a single data center) may score high despite limited labor market depth. Conversely, counties near major metros may have accessible talent that commutes in but isn&apos;t captured here.</p>
                  <p><strong className="text-gray-700 dark:text-gray-200">Coverage:</strong> 100%. 2,222 of 3,143 counties have at least one IT establishment; the remaining 921 score 0.</p>
                </div>
              </SubSection>

              <SubSection title="6. Fiber Availability (fiber_score)">
                <p>Actual fiber-to-the-premises (FTTP) availability from <strong className="text-gray-900 dark:text-white">ISP-reported FCC data</strong> — a direct measure of fiber infrastructure presence at the location level.</p>
                <div className="bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-lg p-4 space-y-2 text-sm">
                  <p><strong className="text-gray-700 dark:text-gray-200">Primary Source:</strong> FCC Broadband Data Collection (BDC), December 2024 vintage — county-level summaries via ArcGIS Living Atlas. Measures the percentage of Broadband Serviceable Locations (BSLs) with fiber availability and the number of competing fiber providers per county.</p>
                  <p><strong className="text-gray-700 dark:text-gray-200">Fallback Source:</strong> Census ACS 5-Year (2023), Table B28002 — broadband subscription rates, used only for counties missing from BDC data.</p>
                  <p><strong className="text-gray-700 dark:text-gray-200">Method:</strong> Composite score = 80% × (fiber BSLs ÷ total BSLs) + 20% × (provider competition, capped at 5). Percentile rank normalization across all counties. Provider competition captures market depth — a county with 3 fiber ISPs has more robust infrastructure than one with a single provider.</p>
                  <p><strong className="text-gray-700 dark:text-gray-200">What it measures:</strong> The share of locations in a county where at least one ISP reports fiber-to-the-premises availability, plus the competitive landscape. This is ISP-reported data filed with the FCC — a significant improvement over the previous broadband subscription proxy.</p>
                  <p><strong className="text-gray-700 dark:text-gray-200">Limitations:</strong> ISP-reported data may overstate actual availability (ISPs sometimes report planned coverage). This measures consumer/business FTTP, not enterprise dark fiber, lit fiber routes, or carrier-neutral interconnection points. A county with high residential fiber may still lack the dedicated dark fiber infrastructure data centers need. However, counties with extensive FTTP deployment almost always have better underlying fiber trunk infrastructure.</p>
                  <p><strong className="text-gray-700 dark:text-gray-200">Coverage:</strong> ~3,234 counties via FCC BDC (primary), remainder via ACS fallback.</p>
                </div>
              </SubSection>
            </div>
          </Section>
          ...
          <Section id="normalization" title="Normalization">
            ...
            <SubSection title="2. Percentile Rank">
              ...
              <p>For inverse metrics (SAIDI): <code className="text-nodiac-secondary bg-gray-100 dark:bg-white/5 px-1 rounded">1 − rank</code>.</p>
            </SubSection>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-200 dark:border-white/10">
                    ...
                    <th className="text-left py-2 pl-3 text-gray-500 dark:text-gray-400 font-medium">Rationale</th>
                  </tr>
                </thead>
                <tbody className="font-mono">
                  <tr className="border-b border-gray-100 dark:border-white/5">
                    ...
                    <td className="text-center py-2 px-2">1.0</td>
                    <td className="text-center py-2 px-2">1.0</td>
                    <td className="py-2 pl-3 font-sans text-gray-500 dark:text-gray-400 text-xs">Equal weight baseline</td>
                  </tr>
                  <tr className="border-b border-gray-100 dark:border-white/5">
                    ...
                    <td className="text-center py-2 px-2 text-gray-400 dark:text-gray-500">0.5</td>
                    <td className="text-center py-2 px-2">1.0</td>
                    <td className="py-2 pl-3 font-sans text-gray-500 dark:text-gray-400 text-xs">Core thesis: co-op territories + permitting</td>
                  </tr>
                  <tr className="border-b border-gray-100 dark:border-white/5">
                    ...
                    <td className="text-center py-2 px-2">1.0</td>
                    <td className="text-center py-2 px-2 text-nodiac-secondary font-bold">2.0</td>
                    <td className="py-2 pl-3 font-sans text-gray-500 dark:text-gray-400 text-xs">Time-to-power constraint</td>
                  </tr>
                  <tr>
                    ...
                    <td className="text-center py-2 px-2 text-gray-400 dark:text-gray-500">0.5</td>
                    <td className="text-center py-2 px-2">1.0</td>
                    <td className="py-2 pl-3 font-sans text-gray-500 dark:text-gray-400 text-xs">Renewable arbitrage opportunity</td>
                  </tr>
                </tbody>
              </table>
            </div>
  ...
  weights: {
    ...
    fiber: 0.5,
            ...
            <div className="bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-lg p-4">
              <div className="space-y-4 text-sm">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-gray-900 dark:text-white font-medium">CSV Upload</p>
                    <p className="text-gray-500 dark:text-gray-400">File parsed by <code className="text-nodiac-secondary bg-gray-100 dark:bg-white/5 px-1 rounded">parseFleetCSV()</code> → extracts site_name, lat/lon, utility info, raw data.</p>
                    <p className="text-gray-500 dark:text-gray-400">Sites inserted into <code className="text-nodiac-secondary bg-gray-100 dark:bg-white/5 px-1 rounded">portfolio_sites</code> table with county/state from raw CSV data.</p>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-gray-900 dark:text-white font-medium">FIPS Resolution</p>
                    <p className="text-gray-500 dark:text-gray-400">Three-strategy cascade: (a) county+state name match against county_scores table, (b) FCC Area API batch lookup by lat/lon, (c) null if unresolvable.</p>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-gray-900 dark:text-white font-medium">Score Lookup</p>
                    <p className="text-gray-500 dark:text-gray-400">FIPS code → county_scores row. The site inherits all six criterion scores from its county.</p>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-gray-900 dark:text-white font-medium">Scoring & Tiering</p>
                    <p className="text-gray-500 dark:text-gray-400">Server-side: Balanced (equal weight) average → score + tier. Client-side: real-time re-scoring with any weight profile.</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <Section id="fips-resolution" title="FIPS Resolution">
            ...
            <ol className="list-decimal list-inside space-y-2 ml-2">
              <li><strong className="text-gray-900 dark:text-white">County + State name match</strong> — Normalized lowercase lookup against the county_scores table. Fast, no API call.</li>
              <li><strong className="text-gray-900 dark:text-white">FCC Area API</strong> — <code className="text-nodiac-secondary bg-gray-100 dark:bg-white/5 px-1 rounded">geo.fcc.gov/api/census/area?lat=X&lon=Y</code> — returns county FIPS from coordinates. Batched with concurrency limit of 5.</li>
              <li><strong className="text-gray-900 dark:text-white">Null</strong> — If both fail, the site gets null scores (unscored).</li>
            </ol>
...
→ { results: [{ county_fips: "06075", county_name: "San Francisco", ... }] }`}</CodeBlock>
          ...
          <Section id="map-rendering" title="Map Rendering">
            <SubSection title="Choropleth (Regional Hubs)">
              <p>
                ...
                Composite scores are injected into each feature&apos;s <code className="text-nodiac-secondary bg-gray-100 dark:bg-white/5 px-1 rounded">properties.compositeScore</code> field,
                then Mapbox&apos;s <code className="text-nodiac-secondary bg-gray-100 dark:bg-white/5 px-1 rounded">interpolate</code> expression determines fill color.
              </p>
              <p className="text-sm text-gray-500 dark:text-gray-400">
                This avoids the Mapbox <code className="text-nodiac-secondary bg-gray-100 dark:bg-white/5 px-1 rounded">match</code> expression size limit (~3,000 entries).
              </p>
            </SubSection>
            ...
            <SubSection title="Site Markers (Screening)">
              <p>
                Sites render as colored circle markers using <code className="text-nodiac-secondary bg-gray-100 dark:bg-white/5 px-1 rounded">react-map-gl Marker</code> components.
          ...
          <Section id="api-reference" title="API Reference">
            <div className="space-y-6">
              <div className="bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-lg p-4">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400">Returns all ~3,200 county scores. Cached for 1 hour (s-maxage=3600). Falls back to static JSON if Supabase unavailable.</p>
              </div>
              <div className="bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-lg p-4">
                <p className="font-mono text-sm text-nodiac-secondary mb-2">GET /api/hub-regions</p>
                <p className="text-sm text-gray-500 dark:text-gray-400">Returns hub region GeoJSON overlays for the map.</p>
              </div>
              <div className="bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-lg p-4">
                <p className="font-mono text-sm text-nodiac-secondary mb-2">POST /api/upload-csv</p>
                <p className="text-sm text-gray-500 dark:text-gray-400">Upload a CSV file. Requires auth. Returns upload_id and site_count. Body: FormData with &ldquo;file&rdquo; and optional &ldquo;name&rdquo;.</p>
              </div>
              <div className="bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-lg p-4">
                <p className="font-mono text-sm text-nodiac-secondary mb-2">POST /api/portfolio/[id]/score</p>
                <p className="text-sm text-gray-500 dark:text-gray-400">Triggers FIPS resolution + scoring for all sites in the upload. Returns scored results. Requires auth.</p>
              </div>
              <div className="bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-lg p-4">
                <p className="font-mono text-sm text-nodiac-secondary mb-2">GET /api/portfolio/[id]</p>
                <p className="text-sm text-gray-500 dark:text-gray-400">Returns upload metadata + all sites with scores. Requires auth.</p>
              </div>
            </div>
          </Section>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-200 dark:border-white/10">
                    ...
                    <th className="text-left py-2 text-gray-500 dark:text-gray-400 font-medium">Used For</th>
                  </tr>
                </thead>
                <tbody>
                  <tr className="border-b border-gray-100 dark:border-white/5">
                  ...
                  <tr>
                    <td className="py-2 pr-3 text-gray-900 dark:text-white">FCC Area API</td>
          ...
          <Section id="limitations" title="Known Limitations">
            <ul className="space-y-3 ml-2">
              <li className="flex items-start gap-3">
                ...
                <span><strong className="text-gray-900 dark:text-white">Permitting is state-level</strong> — Scores are primarily state-level with county adjustments where moratorium/opposition data exists. Granular county-by-county zoning research would further improve accuracy.</span>
              </li>
              <li className="flex items-start gap-3">
                <span className="w-1.5 h-1.5 rounded-full bg-yellow-400 mt-2 flex-shrink-0" />
                <span><strong className="text-gray-900 dark:text-white">Curtailment is a proxy</strong> — Measures installed MW, not actual curtailment. CAISO reports ~3.4M MWh curtailed in 2024. Adding EIA Form 923 capacity factor gaps would improve this.</span>
              </li>
              <li className="flex items-start gap-3">
                <span className="w-1.5 h-1.5 rounded-full bg-yellow-400 mt-2 flex-shrink-0" />
                <span><strong className="text-gray-900 dark:text-white">Fiber measures FTTP availability, not enterprise dark fiber</strong> — FCC BDC data captures ISP-reported fiber-to-the-premises coverage, which correlates with but doesn&apos;t directly measure enterprise/dark fiber infrastructure. <strong className="text-gray-900 dark:text-white">Labor is a proxy</strong> — CBP business counts ≠ hireable labor supply. Both are the best freely available county-level data.</span>
              </li>
              <li className="flex items-start gap-3">
                <span className="w-1.5 h-1.5 rounded-full bg-yellow-400 mt-2 flex-shrink-0" />
                <span><strong className="text-gray-900 dark:text-white">No interaction effects</strong> — Weighted average treats criteria independently. A county with high co-op density AND high curtailment is more than additively valuable. Geometric mean or multiplicative model could capture this.</span>
              </li>
              <li className="flex items-start gap-3">
                <span className="w-1.5 h-1.5 rounded-full bg-yellow-400 mt-2 flex-shrink-0" />
                <span><strong className="text-gray-900 dark:text-white">Census noise</strong> — CBP employment counts have 2–5% noise infusion for disclosure avoidance.</span>
              </li>
              <li className="flex items-start gap-3">
                <span className="w-1.5 h-1.5 rounded-full bg-yellow-400 mt-2 flex-shrink-0" />
                <span><strong className="text-gray-900 dark:text-white">46% zero curtailment</strong> — Counties with no variable renewables score 0.0. Bimodal distribution can distort the map when curtailment weight is high.</span>
              </li>
              <li className="flex items-start gap-3">
                <span className="w-1.5 h-1.5 rounded-full bg-yellow-400 mt-2 flex-shrink-0" />
                <span><strong className="text-gray-900 dark:text-white">Mostly static snapshot</strong> — Grid reliability now uses multi-year averages (2013–2024), but curtailment and permitting are still single-year snapshots.</span>
              </li>
            </ul>
          </Section>
          ...
          <Section id="extending" title="Extending the Model">
            <SubSection title="Adding a New Criterion">
              <ol className="list-decimal list-inside space-y-2 ml-2">
                <li>Add the key to <code className="text-nodiac-secondary bg-gray-100 dark:bg-white/5 px-1 rounded">CriterionKey</code> union type in <code className="text-nodiac-secondary bg-gray-100 dark:bg-white/5 px-1 rounded">src/types/regional-hubs.ts</code></li>
                <li>Add label + description to <code className="text-nodiac-secondary bg-gray-100 dark:bg-white/5 px-1 rounded">CRITERION_LABELS</code> and <code className="text-nodiac-secondary bg-gray-100 dark:bg-white/5 px-1 rounded">CRITERION_DESCRIPTIONS</code></li>
                <li>Add <code className="text-nodiac-secondary bg-gray-100 dark:bg-white/5 px-1 rounded">[key]_score: number</code> field to <code className="text-nodiac-secondary bg-gray-100 dark:bg-white/5 px-1 rounded">CountyScore</code> interface</li>
                <li>Add to <code className="text-nodiac-secondary bg-gray-100 dark:bg-white/5 px-1 rounded">ALL_CRITERIA</code> array</li>
                <li>Update <code className="text-nodiac-secondary bg-gray-100 dark:bg-white/5 px-1 rounded">getCriterionValue()</code> in <code className="text-nodiac-secondary bg-gray-100 dark:bg-white/5 px-1 rounded">county-scorer.ts</code></li>
                <li>Add default weight to all profiles in <code className="text-nodiac-secondary bg-gray-100 dark:bg-white/5 px-1 rounded">weight-profiles.ts</code></li>
                <li>Add to <code className="text-nodiac-secondary bg-gray-100 dark:bg-white/5 px-1 rounded">SiteScoreBreakdown</code> in <code className="text-nodiac-secondary bg-gray-100 dark:bg-white/5 px-1 rounded">screening.ts</code></li>
                <li>Add to <code className="text-nodiac-secondary bg-gray-100 dark:bg-white/5 px-1 rounded">buildSiteBreakdown()</code> in <code className="text-nodiac-secondary bg-gray-100 dark:bg-white/5 px-1 rounded">site-scorer.ts</code></li>
                <li>Add column to <code className="text-nodiac-secondary bg-gray-100 dark:bg-white/5 px-1 rounded">county_scores</code> Supabase table</li>
            ...
            <SubSection title="Planned Upgrades">
              <ul className="list-disc list-inside space-y-1 ml-2 text-sm">
                <li><strong className="text-gray-700 dark:text-gray-200">EIA Form 923</strong> — Generation data for capacity factor gap analysis (better curtailment proxy)</li>
                <li><strong className="text-gray-700 dark:text-gray-200">Long-haul fiber routes</strong> — Proximity to backbone fiber (e.g., InterTubes dataset) for enterprise/dark fiber scoring</li>
                <li><strong className="text-gray-700 dark:text-gray-200">Permitting enrichment</strong> — Batch web research via Claude skill, prioritizing target hub regions</li>
                <li><strong className="text-gray-700 dark:text-gray-200">Temporal tracking</strong> — Store score history, show trends over time</li>
                <li><strong className="text-gray-700 dark:text-gray-200">Geometric mean option</strong> — Capture interaction effects between criteria</li>
              </ul>
            </SubSection>
          </Section>
        </main>
