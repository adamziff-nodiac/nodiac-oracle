...
export function registerWriteTools(server: McpServer, getClient: () => SupabaseClient, getUserEmail: () => string) {
  ...
  server.tool(
    ...
    async ({ site_id, apn, area_acres, landowner_id, notes }) => {
      ...
      await logActivity(supabase, site_id, 'Parcel added', `APN: ${apn ?? 'N/A'}, ${area_acres ?? '?'} acres`, getUserEmail())
  ...
  server.tool(
    ...
    async ({ site_id, landowner_id, proximity, purpose, lease_status }) => {
      ...
      await logActivity(supabase, site_id, 'Landowner linked', `Landowner ${landowner_id} linked (${proximity})`, getUserEmail())
  ...
  server.tool(
    ...
    async ({ site_id, landowner_id }) => {
      ...
      await logActivity(supabase, site_id, 'Landowner unlinked', `Landowner ${landowner_id} removed`, getUserEmail())
  ...
  server.tool(
    ...
    async ({ parcel_id }) => {
      ...
      if (parcel?.site_id) {
        await logActivity(supabase, parcel.site_id, 'Parcel deleted', `Parcel ${parcel_id} removed`, getUserEmail())
      }
