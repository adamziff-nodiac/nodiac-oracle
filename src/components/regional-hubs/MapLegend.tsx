  ...
  return (
    <div className="absolute bottom-4 right-4 bg-white/80 dark:bg-nodiac-dark/80 backdrop-blur-xl border border-gray-200 dark:border-white/10 rounded-lg px-4 py-3 z-10">
      ...
      <div className="flex items-center gap-2">
        <span className="text-xs text-gray-500 dark:text-gray-400 tabular-nums">{min.toFixed(1)}</span>
        <div
        ...
        />
        <span className="text-xs text-gray-500 dark:text-gray-400 tabular-nums">{max.toFixed(1)}</span>
      </div>
    </div>
  )
}
