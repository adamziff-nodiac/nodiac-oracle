  ...
  return (
    <div className="border border-gray-100 dark:border-white/5 rounded-lg overflow-hidden">
      <button
        ...
        className="w-full flex items-center justify-between px-3 py-2 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider hover:bg-gray-50 dark:hover:bg-white/5 transition-colors"
      >
        ...
        <div className={cn('px-3 pb-3 space-y-1.5', compact && 'max-h-48 overflow-y-auto')}>
          {sortedCitations.map((c, i) => (
            <a
              ...
              className="flex items-start gap-2 group py-1 hover:bg-gray-50 dark:hover:bg-white/5 rounded px-1 -mx-1 transition-colors"
            >
              <ExternalLink className="w-3 h-3 mt-0.5 text-gray-500 group-hover:text-nodiac-secondary flex-shrink-0" />
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5 flex-wrap">
                  <TypeBadge type={c.type} />
                  <span className="text-xs text-gray-600 dark:text-gray-300 group-hover:text-gray-900 dark:group-hover:text-white transition-colors truncate">
                    {c.title}
                  </span>
                </div>
                {!compact && c.relevance && (
