...
function SlideBackground({ children }: { children: React.ReactNode }) {
  return (
    <AbsoluteFill style={{
      ...
      {/* Ambient glow */}
      <div style={{ position: 'absolute', inset: 0, background: `radial-gradient(ellipse at 40% 30%, ${C.teal}06 0%, transparent 50%)` }} />
      {children}
    </AbsoluteFill>
  );
}
...
function TheOneLiner() {
  ...
  return (
    <SlideBackground>
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', opacity: fadeIn }}>
        <div style={{ transform: `scale(${logoSc})` }}>
          <NodiacLogo width={400} />
        </div>
        <div style={{ marginTop: 60, opacity: lineOp, textAlign: 'center', maxWidth: 1100 }}>
          <div style={{ fontSize: 42, fontWeight: 600, color: C.white, lineHeight: 1.4 }}>
    ...
    <SlideBackground>
      <div style={{ opacity: fadeIn }}>
        <div style={{ position: 'absolute', top: 100, left: 140 }}>
          <div style={{ fontSize: 18, fontWeight: 700, color: C.teal, letterSpacing: 5, opacity: headerOp }}>THE PROBLEM</div>
          ...
          {stats.map((s) => {
            ...
            return (
              <div key={s.label} style={{
                ...
                background: 'rgba(0,0,0,0.35)', border: `1px solid ${s.color}20`,
                opacity: sc, transform: `translateY(${(1 - sc) * 20}px)`,
                backdropFilter: 'blur(4px)',
              }}>
                <div style={{ fontSize: 38, fontWeight: 900, color: s.color, fontFamily: 'JetBrains Mono, monospace' }}>{s.value}</div>
                <div style={{ fontSize: 14, color: C.lilac, marginTop: 10, letterSpacing: 1 }}>{s.label}</div>
              </div>
            );
          })}
        </div>
...
function TheThesis() {
  ...
  const headerOp = interpolate(frame, [0, 20], [0, 1], { extrapolateRight: 'clamp' });
  const quoteOp = interpolate(frame, [130, 150], [0, 1], { extrapolateRight: 'clamp' });

  return (
    <SlideBackground>
      <div style={{ opacity: fadeIn }}>
        <div style={{ position: 'absolute', top: 100, left: 140 }}>
          <div style={{ fontSize: 18, fontWeight: 700, color: C.teal, letterSpacing: 5, opacity: headerOp }}>THE THESIS</div>
          <div style={{ fontSize: 46, fontWeight: 800, color: C.white, marginTop: 8, opacity: headerOp }}>
            Bring the load to the generation.
          </div>
        </div>

        <div style={{ position: 'absolute', top: 250, left: 140, right: 140 }}>
          {steps.map((s) => {
            const op = spring({ frame: frame - s.delay, fps, config: { damping: 12 } });
            return (
              <div key={s.text} style={{
                display: 'flex', alignItems: 'center', gap: 24,
                marginBottom: 28, opacity: op, transform: `translateX(${(1 - op) * 40}px)`,
              }}>
                <div style={{ fontSize: 32, width: 52, textAlign: 'center' }}>{s.icon}</div>
                <div style={{ fontSize: 26, color: C.white, fontWeight: 500 }}>{s.text}</div>
              </div>
            );
          })}
        </div>
        ...
        <div style={{ position: 'absolute', bottom: 100, left: 140, right: 300, opacity: quoteOp }}>
          <div style={{ fontSize: 20, color: C.orchid, fontStyle: 'italic', lineHeight: 1.6 }}>
          ...
          <div style={{ fontSize: 16, color: C.lilac, marginTop: 10 }}>
...
function ThePortfolio() {
  ...
  const totalOp = spring({ frame: frame - 90, fps, config: { damping: 12 } });
  const pilotOp = interpolate(frame, [130, 150], [0, 1], { extrapolateRight: 'clamp' });
      ...
      <div style={{ opacity: fadeIn }}>
        ...
        <div style={{ position: 'absolute', top: 260, left: 140, right: 140 }}>
          {stateData.map((s) => {
            ...
            const maxMW = 180;
            return (
              <div key={s.abbr} style={{ marginBottom: 32, opacity: barW }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                    <div style={{ width: 16, height: 16, borderRadius: '50%', background: s.color }} />
                    <span style={{ fontSize: 28, fontWeight: 800, color: C.white }}>{s.abbr}</span>
                  </div>
        ...
        <div style={{ position: 'absolute', bottom: 180, left: 140, display: 'flex', gap: 80, opacity: totalOp }}>
          <div>
            <div style={{ fontSize: 64, fontWeight: 900, color: C.teal }}>{TOTAL_SITES}</div>
            <div style={{ fontSize: 18, color: C.lilac, letterSpacing: 3 }}>TOTAL SITES</div>
          </div>
          <div>
            <div style={{ fontSize: 64, fontWeight: 900, color: C.orchid }}>{TOTAL_CAP}</div>
            <div style={{ fontSize: 18, color: C.lilac, letterSpacing: 3 }}>MW CAPACITY</div>
          </div>
          <div>
            <div style={{ fontSize: 64, fontWeight: 900, color: C.white }}>3</div>
            <div style={{ fontSize: 18, color: C.lilac, letterSpacing: 3 }}>STATES</div>
          </div>
        </div>

        <div style={{ position: 'absolute', bottom: 80, left: 140, right: 140, opacity: pilotOp }}>
          <div style={{ fontSize: 20, color: C.lilac }}>
            Pilot sites at <span style={{ color: C.teal, fontWeight: 700 }}>Hay River</span> and <span style={{ color: C.teal, fontWeight: 700 }}>Walleye</span> (WI) in active development with Dunn Energy Cooperative.
          </div>
        </div>
      </div>
    </SlideBackground>
...
function TheBusinessModel() {
  ...
  return (
    <SlideBackground>
      <div style={{ opacity: fadeIn }}>
        <div style={{ position: 'absolute', top: 100, left: 140 }}>
          <div style={{ fontSize: 18, fontWeight: 700, color: C.teal, letterSpacing: 5, opacity: headerOp }}>UNIT ECONOMICS</div>
          <div style={{ fontSize: 46, fontWeight: 800, color: C.white, marginTop: 8, opacity: headerOp }}>
            Triple-net lease. Stable EBITDA.
          </div>
        </div>

        <div style={{ position: 'absolute', top: 250, left: 120, right: 120, display: 'flex', gap: 24 }}>
          {metrics.map((m) => {
            const sc = spring({ frame: frame - m.delay, fps, config: { damping: 12 } });
            return (
              <div key={m.label} style={{
                flex: 1, padding: '36px 28px', borderRadius: 20,
                background: 'rgba(0,0,0,0.35)', border: `2px solid ${m.color}20`,
                textAlign: 'center', opacity: sc, transform: `translateY(${(1 - sc) * 30}px)`,
                backdropFilter: 'blur(4px)',
              }}>
                <div style={{ fontSize: 48, fontWeight: 900, color: m.color, fontFamily: 'JetBrains Mono, monospace' }}>{m.value}</div>
                <div style={{ fontSize: 14, color: C.lilac, marginTop: 14, letterSpacing: 1.5 }}>{m.label}</div>
              </div>
            );
          })}
        </div>

        <div style={{ position: 'absolute', bottom: 100, left: 140, right: 140, opacity: modelOp }}>
          <div style={{ display: 'flex', justifyContent: 'center', gap: 60 }}>
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: 20, fontWeight: 700, color: C.white }}>Contract Structure</div>
              <div style={{ fontSize: 16, color: C.lilac, marginTop: 6 }}>5-10 year MSAs &bull; Tier-1 counterparty</div>
            </div>
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: 20, fontWeight: 700, color: C.white }}>Revenue Model</div>
              <div style={{ fontSize: 16, color: C.lilac, marginTop: 6 }}>Monthly flat rate &bull; $/MW/month</div>
            </div>
            <div style={{ textAlign: 'center' }}>
              <div style={{ fontSize: 20, fontWeight: 700, color: C.white }}>Deployment</div>
              <div style={{ fontSize: 16, color: C.lilac, marginTop: 6 }}>Armada modular pods &bull; Trailer-mounted</div>
            </div>
          </div>
        </div>
      </div>
...
function TheScalingPlan() {
  ...
  const diffOp = interpolate(frame, [170, 190], [0, 1], { extrapolateRight: 'clamp' });

  return (
    <SlideBackground>
      <div style={{ opacity: fadeIn }}>
        <div style={{ position: 'absolute', top: 100, left: 140 }}>
          <div style={{ fontSize: 18, fontWeight: 700, color: C.teal, letterSpacing: 5, opacity: headerOp }}>SCALING ROADMAP</div>
          <div style={{ fontSize: 46, fontWeight: 800, color: C.white, marginTop: 8, opacity: headerOp }}>
        ...
        <div style={{ position: 'absolute', top: 250, left: 140, right: 140 }}>
          {milestones.map((m) => {
            ...
            return (
              <div key={m.year} style={{ marginBottom: 44, opacity: barW }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
                  <span style={{ fontSize: 28, fontWeight: 800, color: C.white }}>{m.year}</span>
                  <div style={{ display: 'flex', gap: 28 }}>
                    <span style={{ fontSize: 24, fontWeight: 800, color: m.color }}>{m.mw}</span>
                    <span style={{ fontSize: 24, fontWeight: 700, color: C.lilac }}>{m.rev}</span>
                  </div>
                </div>
                <div style={{ height: 28, borderRadius: 10, background: 'rgba(0,0,0,0.4)' }}>
                  <div style={{
...
function InvestorCTA() {
  ...
  return (
    <SlideBackground>
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', opacity: fadeIn * fadeOut }}>
        <div style={{ transform: `scale(${logoSc})` }}>
          <NodiacLogo width={500} />
        </div>

        <div style={{ marginTop: 50, opacity: teamOp, textAlign: 'center' }}>
          <div style={{ fontSize: 22, fontWeight: 600, color: C.orchid }}>
            Built by the team behind Greenbacker ($3B AUM)
          </div>
          <div style={{ fontSize: 18, color: C.lilac, marginTop: 10 }}>
